...
class NaturalGradient(tf.train.Optimizer):
    def __init__(self, gamma: Scalar, name=None):
        name = self.__class__.__name__ if name is None else name
        super(NaturalGradient, self).__init__(use_locking=False, name=name)
        self.gamma = gamma
    ...
    def minimize(self, loss_fn: Callable, var_list: List[Parameter]):
        ...
        return self._natgrad_steps(loss_fn, parameters)
    ...
    def _natgrad_steps(self, loss_fn: Callable, parameters: List[Tuple[Parameter, Parameter, XiTransform]]):
        def natural_gradient_step(q_mu, q_sqrt, xi_transform):
            return self._natgrad_step(loss_fn, q_mu, q_sqrt, xi_transform)

        with tf.name_scope(f"{self._name}/natural_gradient_steps"):
            res = list(map(natural_gradient_step, *zip(*parameters)))
            return tf.group(res)
    ...
    def _natgrad_step(self, loss_fn: Callable, q_mu: Parameter, q_sqrt: Parameter, xi_transform: XiTransform):
        ...
        # with tf.GradientTape(persistent=True, watch_accessed_variables=False) as tape:
            # tape.watch([q_mu.vf_val, q_sqrt.vf_val])

        loss = loss_fn()
        dL_dmean, dL_dvarsqrt = tf.gradients(loss, [q_mu.vf_val, q_sqrt.vf_val])
        return self._natgrad_step_from_grad(dL_dmean, dL_dvarsqrt, q_mu, q_sqrt, xi_transform)
    ...
    def _natgrad_step_from_grad(self, dL_dmean, dL_dvarsqrt, q_mu: Parameter, q_sqrt: Parameter, xi_transform: XiTransform):
        """
        Implements equation [10] from
        ...
        eta1, eta2 = meanvarsqrt_to_expectation(q_mu.value, q_sqrt.value)
        ...
        if not isinstance(xi_transform, XiNat):
            ...
            nat1, nat2 = meanvarsqrt_to_natural(q_mu.value, q_sqrt.value)
        ...
        # dL_dmean, dL_dvarsqrt = tf.gradients(loss, [q_mu.vf_val, q_sqrt.vf_val])
        dL_dvarsqrt = q_sqrt.transform.forward_tensor(dL_dvarsqrt)
        ...
        dL_deta1, dL_deta2 = tf.gradients(meanvarsqrt, [eta1, eta2], grad_ys=[dL_dmean, dL_dvarsqrt])
        ...
        xi1, xi2 = xi_transform.meanvarsqrt_to_xi(q_mu.value, q_sqrt.value)
        ...
        return tf.group([q_mu.assign(mean_new), q_sqrt.assign(varsqrt_new)])


class EfficientNaturalGradient(NaturalGradient):

    def _natgrad_steps(self, loss_fn: Callable, parameters: List[Tuple[Parameter, Parameter, XiTransform]]):

        with tf.name_scope(f"{self._name}/natural_gradient_steps"):
            L = len(parameters)
            mus = [ps[0].vf_val for ps in parameters]
            q_sqrts = [ps[1].vf_val for ps in parameters]
            dL_params = tf.gradients(loss_fn(), mus + q_sqrts)
            dL_augmented_params = [tuple([dm, ds] + list(p)) for dm, ds, p in zip(dL_params[: L], dL_params[L: ], parameters)]
            res = list(map(self._natgrad_step_from_grad, *zip(*dL_augmented_params)))
            return tf.group(res)
...
def natural_to_meanvarsqrt(nat1: tf.Tensor, nat2: tf.Tensor):
    ...
    return mu, tf.transpose(tf.linalg.cholesky(S), [1, 2, 0])


@swap_dimensions
def meanvarsqrt_to_natural(mu: tf.Tensor, s_sqrt: tf.Tensor):
    s_sqrt = tf.transpose(s_sqrt, (2, 0, 1))
...
def expectation_to_meanvarsqrt(eta1: tf.Tensor, eta2: tf.Tensor):
    ...
    return eta1, tf.transpose(tf.linalg.cholesky(var), [1, 2, 0])


@swap_dimensions
def meanvarsqrt_to_expectation(m: tf.Tensor, v_sqrt: tf.Tensor):
    v_sqrt = tf.transpose(v_sqrt, (2, 0, 1))
