...
from global_settings.constants import RESULT_AL_PATH, LOG_AL_PREDICTION_PATH, RESULT_AL_SELECTION_PATH, ORACLE_CKPT_DIR
...
def run(args):
    log_dir = osp.join(LOG_AL_PREDICTION_PATH, '%s_%s'%(args.dataset, 'oracle'))
    ...
    base_method = 'gp' if args.base_method == 'svgp' else args.base_method
    result_dir = osp.join(RESULT_AL_PATH, '%s_%s_%s.json'%(args.dataset, 'oracle', base_method))
