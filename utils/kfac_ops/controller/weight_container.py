...
class WeightContainer(object):
    ...
    def _combine_weight_bias(self):
        ...
        """
        # with tf.control_dependencies([tf.check_numerics(self._weight, message='weight'+str(self._idx)),
        #                               tf.check_numerics(self._bias, message='bias'+str(self._idx)),
        #                               tf.print('weight00', self._weight[0][0])]):
        weight = tf.reshape(self._weight, (self._in_channels - 1, self._out_channels)) + 0.
...
class MVGWeightContainer(WeightContainer):
    ...
    def sample_weight(self, particles):
        ...
        return multi_weight + tf.matmul(self._u, tf.matmul(noise, self._v, transpose_b=True))

    def update_weight(self, fisher_block):
        """ Update the weight of variational posterior.
        :return: Update operations.
        """
        input_coeff = self._coeff
        # Configure damping parameters.
        input_cov, output_cov = fisher_block._input_factor.get_cov(), fisher_block._output_factor.get_cov()
        pi = _compute_pi_tracenorm(input_cov, output_cov)
        coeff = (self._coeff / fisher_block._renorm_coeff) ** 0.5
        damping = coeff / (self._eta ** 0.5)
        chol_input_cov = tf.cholesky(input_cov + damping * pi * tf.eye(self._u.shape.as_list()[0]))
        chol_output_cov = tf.cholesky(output_cov + damping / pi * tf.eye(self._v.shape.as_list()[0]))

        # Upper triangular
        new_u_sqrt = tf.transpose(tf.linalg.triangular_solve(
            chol_input_cov,
            tf.eye(chol_input_cov.shape.as_list()[0]))) * input_coeff**0.25

        # Upper triangular
        new_v_sqrt = tf.transpose(tf.linalg.triangular_solve(
            chol_output_cov,
            tf.eye(chol_output_cov.shape.as_list()[0]))) * input_coeff**0.25

        uv_sqrt_update_ops = [self._u.assign(new_u_sqrt), self._v.assign(new_v_sqrt)]
        return tf.group(*uv_sqrt_update_ops)
