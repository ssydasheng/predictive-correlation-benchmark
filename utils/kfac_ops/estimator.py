...
class FisherEstimator(object):
    ...
    def _setup(self, cov_ema_decay, scale_ema_decay, distribution):
        ...
        try:
            ...
            # with tf.control_dependencies([tf.check_numerics(tensors_to_compute_grads[0][0], message='input tensor 1'),
            #                               tf.check_numerics(tensors_to_compute_grads[1][0], message='input tensor 2')]):
        ...
        for idx, (grads_list, fb) in enumerate(zip(grads_lists, fisher_blocks_list)):
            # import tensorflow as tf
            # with tf.control_dependencies([tf.check_numerics(grads_list[0], message='grads_list here %d' % idx)]):
            #     A = grads_list[0][0] + 0.
            #     grads_list = ([A],)
