...
from utils.kfac_ops import estimator as est
...
class NGOptimizer(gradient_descent.GradientDescentOptimizer):
    ...
    def _compute_update_steps(self, grads_and_vars):
        if self._momentum_type == "regular":
            ...
            # with tf.control_dependencies([tf.check_numerics(g, message='ggg %s'%v.name) for g, v in grads_and_vars]):
            ...
            if self._norm_constraint is not None:
                ...
                #         [tf.check_numerics(g, message='ggg %s' % v.name) for g, v in precon_grads_and_vars]):
