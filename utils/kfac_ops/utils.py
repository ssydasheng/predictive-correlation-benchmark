...
def posdef_inv(tensor, damping, INV_METHOD=None):
    if INV_METHOD is None:
        INV_METHOD = POSDEF_INV_METHOD
    ...
    return posdef_inv_functions[INV_METHOD](tensor, identity, damping)
...
def posdef_inv_eig(tensor, identity, damping):
    ...
    eigenvalues, eigenvectors = tf.to_float(eigenvalues), tf.to_float(eigenvectors)
...
def compute_eigmvg_stdnormal_kl_divergence(mean1, eu1, ev1, var2):
    # (v1 ev1 v1^T) \otimes (u1 eu1 u1^T)
    n, m = tf.shape(mean1)[0], tf.shape(mean1)[1]

    n, m = tf.to_float(n), tf.to_float(m)
    u1_logdet = m * tf.reduce_sum(tf.log(eu1))
    v1_logdet = n * tf.reduce_sum(tf.log(ev1))
    logdet_1 = u1_logdet + v1_logdet

    logdet_2 = tf.log(var2) * n * m

    logdet_difference = logdet_2 - logdet_1
    const = tf.to_float(n * m)

    mean_diff = tf.reduce_sum(mean1**2.) / var2

    trace = tf.reduce_sum(eu1) * tf.reduce_sum(ev1) / var2
